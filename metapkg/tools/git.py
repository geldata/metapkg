...
class GitClone(Git):
    @classmethod
    def initial_clone(cls, repo_url: str, repo_dir: pathlib.Path) -> GitClone:
        cmd(
            "git",
            "clone",
            "--quiet",
    ...
    def normalize_remote_ref(self, ref: str) -> str:
        if ref == "HEAD":
        ...
        if tag_ref in remote_refs or peeled_tag_ref in remote_refs:
            ...
            # Either a SHA or some other refspec, return as-is.
            return ref
    ...
    def update(
        ...
        *,
        ...
        if branch is not None:
            self.run("switch", "--force-create", branch, "FETCH_HEAD")
        else:
            self.run("switch", "--detach", "FETCH_HEAD")
...
def clone_repo(
    ...
    *,
    ...
    if not repo_dir.exists():
        repo = GitClone.initial_clone(repo_url, repo_dir)
